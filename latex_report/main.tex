...
\begin{center}
    ...
    {\Large Gourab Roy}\\[0.2cm]
    {\large gourab.roy@estudiante.uam.es}
\end{center}
\end{titlepage}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.55\textwidth]{section_2/best_matches.png}
...
The estimated homography and fundamental matrix for View\_02 to View\_03 are:
...
Figure~\ref{fig:sec2_pano_vgg} shows the panorama obtained with $\mathbf{H}$ and
the \texttt{vgg\_gui\_F.m} screenshot used to inspect the epipolar geometry.

\begin{figure}[!htbp]
    \centering
    \includegraphics[height=0.17\textheight]{section_2/best_panorama.png}
    \hfill
    \includegraphics[height=0.17\textheight]{section_2/vgg_gui_F_best_pair_screenshot.png}
    \caption{Left: panorama from $\mathbf{H}$.
             Right: \texttt{vgg\_gui\_F.m} screenshot confirming the estimated
             epipolar geometry from $\mathbf{F}$.}
...
an RGB-painted improved point cloud, where each 3D point is coloured from its
reference-view pixel.  The interactive MATLAB figures were saved as
...
\begin{figure}[htbp]
    ...
    \caption{RGB-painted improved Euclidean point cloud.}
...
In this work, I completed the full process from camera calibration to 3D reconstruction.
From Section~1, I found that the screen-checkerboard calibration was the better choice
for the rest of the project because it used more points and gave more stable intrinsic
parameters than the manually clicked floor-tile pattern. This calibration was then scaled
and reused for the scene images in Sections~2 and~3.

In Section~2, I compared several feature matching options and selected the SIFT MATLAB
strict setup on View\_02--View\_03 because it gave the strongest support for the
fundamental matrix. In Section~3, this choice produced 672 multi-view tracks and allowed
the scene to be reconstructed using all five views. The final Euclidean reconstruction
gave a mean reprojection error of $4.465095\,\mathrm{px}$. Overall, the results are
consistent with the expected pipeline: calibration, matching, projective reconstruction,
and finally a Euclidean 3D point cloud.

\section*{Acknowledgement}

I used LLM assistance for structuring the code and for improving the grammar and clarity of the writing in the report.
